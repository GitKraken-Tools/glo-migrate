...
export const trello = (creator) => {
    ...
    return {
        ...
        label: (boardId, name, color) => new Promise((resolve, reject) => {
            Auth(null, creator.gitkrakenId).getProtectedResource(`https://api.trello.com/1/boards/${boardId}/labels?name=${name}&color=${color}`, "POST", tokens.accessToken, tokens.accessTokenSecret, function (error, data, response) {
                resolve(JSON.parse(data));
            });
        }),
        card: (listId, name, desc, due, memberIds, labelIds) => new Promise((resolve, reject) => {
            ...
            Auth(null, creator.gitkrakenId).getProtectedResource(url, "POST", tokens.accessToken, tokens.accessTokenSecret, function (error, data, response) {
                resolve(JSON.parse(data));
            });
        }),
        checklist: (cardId) => new Promise((resolve, reject) => {
            Auth(null, creator.gitkrakenId).getProtectedResource(`https://api.trello.com/1/checklists?idCard=${cardId}&name=Task List`, "POST", tokens.accessToken, tokens.accessTokenSecret, function (error, data, response) {
                resolve(JSON.parse(data));
            });
        }),
        checkitem: (checklistId, name, checked) => new Promise((resolve, reject) => {
            Auth(null, creator.gitkrakenId).getProtectedResource(`https://api.trello.com/1/checklists/${checklistId}/checkItems?name=${name}&checked=${checked}`, "POST", tokens.accessToken, tokens.accessTokenSecret, function (error, data, response) {
                resolve(JSON.parse(data));
            });
        }),
    }
